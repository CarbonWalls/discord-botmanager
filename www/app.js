...
document.getElementById('send-content')?.addEventListener('input', updateSendPreview);
document.getElementById('send-embed-toggle')?.addEventListener('change', e => {
...
document.getElementById('send-attach-btn')?.onclick = () => document.getElementById('send-attachment').click();
document.getElementById('send-attachment')?.addEventListener('change', e => {
...
document.getElementById('log-guild')?.onchange = e => loadLogChannels(e.target.value);
document.getElementById('log-chan')?.onchange = e => { logChannel = e.target.value; document.getElementById('log-rest').disabled = !logChannel; document.getElementById('log-archive').disabled = !logChannel; };

document.getElementById('log-rest')?.onclick = async () => {
...
document.getElementById('log-archive')?.onclick = async () => {
if (!logChannel) return;
hideError(document.getElementById('log-error'));
...
document.getElementById('presence-apply')?.onclick = async () => {
...
document.getElementById('presence-disc')?.onclick = async () => {
...
if (voiceToken) await loadVoiceGuilds();
updateVoiceStatus();
};
if (voiceBot) { await loadVoiceGuilds(); updateVoiceStatus(); }
}
...
document.getElementById('voice-guild')?.onchange = e => loadVoiceChannels(e.target.value);
...
document.getElementById('voice-play-file')?.addEventListener('change', async e => {
...
document.getElementById('voice-play')?.onclick = async () => {
...
document.getElementById('voice-stop')?.onclick = async () => {
if (!voiceBot) return;
voiceOperationInProgress = true;
try {
...
document.getElementById('voice-record-start')?.onclick = async () => {
try {
...
document.getElementById('voice-record-stop')?.onclick = () => {
...
if (cleanToken) await loadCleanGuilds();
};
...
document.getElementById('clean-guild')?.onchange = e => loadCleanChannels(e.target.value);
document.getElementById('clean-chan')?.onchange = e => {
...
document.getElementById('clean-clone')?.onclick = async () => {
...
document.getElementById('clean-delete')?.onclick = async () => {
if (!cleanBot || !cleanChannel) return;
...
document.getElementById('changepw-btn')?.onclick = () => showModal('modal-changepw');
document.getElementById('reset-vault')?.onclick = () => confirmModal(t('settings.reset_confirm'), () => { localStorage.removeItem('v'); lockVault(); location.reload(); });
}

document.getElementById('changepw-form')?.onsubmit = async e => {
e.preventDefault();
...
document.getElementById('lock-btn')?.onclick = () => { lockVault(); tt(t('vault.locked')); };
document.getElementById('menu-toggle')?.onclick = () => document.getElementById('sidebar').classList.toggle('open');
document.getElementById('backdrop')?.onclick = () => document.getElementById('sidebar').classList.remove('open');
document.getElementById('theme-toggle')?.onclick = () => { const themes = ['light','dark','auto']; const cur = document.documentElement.dataset.theme || 'auto'; const next = themes[(themes.indexOf(cur)+1)%3]; document.documentElement.dataset.theme = next; localStorage.setItem('theme', next); document.querySelectorAll('#theme-seg .seg-btn').forEach(b => b.classList.toggle('active', b.dataset.v === next)); };
document.getElementById('add-bot-btn')?.onclick = () => { const n = prompt(t('vault.name_placeholder')); if (!n) return; const tkn = prompt(t('vault.token_placeholder')); if (!tkn) return; addBot(n, tkn).then(() => { renderVault(); tt(t('vault.added_bot')); }); };
