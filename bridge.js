...
async function readBody(req) {
  ...
  for await (const ch of req) {
    size += ch.length;
    if (size > BODY_LIMIT) {
      // no req.destroy() here: the 413 from the handler funnel must reach the
      // client first; node closes the socket once the response is flushed
...
function connectGateway(botId, token) {
  return new Promise((resolve, reject) => {
    ...
    ws.on('close', (code) => {
      ...
      // fatal codes: retrying can never succeed (bad token, bad intents, bad
      // api version) — stay down and let the user fix the cause / reconnect
      if (code === 4004 || code === 4010 || code === 4013) {
        console.error(`[gateway] bot ${botId}: fatal close ${code}, not reconnecting`);
...
function disconnectGateway(botId) {
  ...
  const forget = setTimeout(() => intentionalClose.delete(botId), 10000);
